...
import { ServerMessageModel } from './serverMessage.model';

@Service()
class MongoDBServerMessageReactionsRepo {
    ...
    heartReaction = async (_id: string, userId: string): Promise<ISuccess | BasicError> => {
        try {
            const updatedStatus = await ServerMessageModel.updateOne({ _id}, {
                                                                $push: {
...
export default MongoDBServerMessageReactionsRepo; 