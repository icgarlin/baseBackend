...
class MongoDBUserRepo implements IUserRepo {
    ...
    addToFollowing = async (userId: string, followingUserId: string): Promise<ISuccess | BasicError> => {
        try {
            const res = await UserModel.updateOne({_id: userId}, {
                                                    $push: {
                                                        followingIds: followingUserId
                                                    }
                                                }) as UpdatedDocument; 
            if (res.nModified === 1) return {success:true}; 
            else throw (new BasicError(ErrorCode.BadRequest,`Could not follow user with id ${followingUserId}`)); 
        } catch (error) {
            return error as BasicError; 
        }
    }

    removeFromFollowing = async (userId: string, followingUserId: string): Promise<ISuccess | BasicError> => {
        try {
            const res = await UserModel.updateOne({_id: userId}, {
                                                    $push: {
                                                        followingIds: followingUserId
    ...
    addToFollowers = async (userId: string, followUserId: string): Promise<ISuccess | BasicError> => {
        try {
            const res = await UserModel.updateOne({_id: userId}, {
                                                    $push: {
                                                     followerIds: followUserId
                                                    }
                                                }) as UpdatedDocument; 
            if (res.nModified === 1) return {success:true}; 
            else throw (new BasicError(ErrorCode.BadRequest,`Could not follow user with id ${followUserId}`)); 
        } catch (error) {
            return error as BasicError; 
        }
    }

    removeFromFollowers = async (userId: string, followUserId: string): Promise<ISuccess | BasicError> => {
        try {
            const res = await UserModel.updateOne({_id: userId}, {
                                                    $pull: {
                                                     followerIds: followUserId
    ...
    register = async (info: IRegistration, comparePassword: (password: string, hashedPassword: string) => Promise<boolean | BasicError>, createAccessToken: (userId: string) => string | BasicError, createRefreshToken: (userId: string) => string | BasicError,  createCustomer: (name: string, email: string) => Promise<string | BasicError>): Promise<ILogin | BasicError> => {
        try {
            ...
            if (!userExistsRes) {
               let stripeCustomerId: string | BasicError = 'NONE'; 
               ...
               const newUser = await UserModel.create({
                                                        ...
                                                        email, 
                                                        avatar: ``,
                                                        cover: `common/cover/thndrCoverPhoto.svg`,
                                                        stripeCustomerId
                                                      });
               await newUser.save();
    ...
    /*
    addDMPair = async (fromUserId: string, toUserId: string): Promise<true | BasicError> => {
        try {
           const firstUpdatedUser = await UserModel
            .updateOne(
              { userId: fromUserId },
              { $push: { directMessages: toUserId } }
            ) as UpdatedDocument

            const secondUpdatedUser = await UserModel
            .updateOne(
              { userId: toUserId },
              { $push: { directMessages: fromUserId } }
            ) as UpdatedDocument; 
    ...
    */ 
    ...
    addDMGroup = async (fromId: string, toIds: string[]): Promise<ISuccess | BasicError> => {
        try {
            console.log('the from id ', fromId); 
            console.log('the to ids ', toIds); 
    ...
    // removeSubscriptionInfo = async (userId: string, serverId: string): Promise<ISuccess | BasicError> => {
    //     try {
    //         const res = await UserModel.updateOne({_id: userId, 'serversJoined.serverId': serverId }, {
    //                                                                $unset: { 
    ...
    //                                                 }) as UpdatedDocument; 
    //         if (res.nModified === 1) return {success: true}; 
    //         else throw (new BasicError(ErrorCode.MongoDBError,`Could not remove server ${serverId} subscription info`)); 
    //     } catch (error) {
    //         return error as BasicError; 
    //     }
    // }
    ...
    getConnections = async (userId: string): Promise<IUser[] | BasicError> => {
        try {
            ...
            const sanitizedUserList = userList.filter((val) => val !== undefined && val !== null); 
            console.log('the sanitized list ', sanitizedUserList); 
